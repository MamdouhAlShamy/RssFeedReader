...
void MainWindow::rss_render()
{
    ...
    //    item *new_item=new item[count];
    //    //pointer to use with the items
    //    new_item_pointer=new_item;

    //    ItemWidget *new_item=new ItemWidget[count];
    //    new_item_pointer=new_item;
        ...
        if(xml.isStartElement())
        {
            if(xml.name()=="item")
            {
                ...
                while (!((xml.isEndElement())&&(xml.name()=="item")))
                {
                    ...
                    {
                    case 0://title tag
                        list_of_itemsWidget[i]->title=xml.readElementText(QXmlStreamReader::IncludeChildElements);
                        //adjusting list
                        list_item->setText(list_of_itemsWidget[i]->title);
                        //add Item
                        ...
                        break;
                    case 1://link tag
                        list_of_itemsWidget[i]->link=new QUrl(xml.readElementText(QXmlStreamReader::IncludeChildElements));
                        break;
                    case 2://description tag
                        list_of_itemsWidget[i]->description=xml.readElementText(QXmlStreamReader::IncludeChildElements);
                        break;
                    case 3://pub date tag
                        list_of_itemsWidget[i]->pub_date=xml.readElementText(QXmlStreamReader::IncludeChildElements);
                        break;
                    default:
                        break;
                    }
